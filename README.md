...
3. **Google Kalender Integration**:
   - Ein dedizierter Client in `src/calendar_client.cpp` holt zyklisch über WLAN Termine ab.
   - Das Abrufen geschieht über ein Google Apps Script (JSON Web-API), dekodiert über `ArduinoJson`.
   - Das Intervall für den Abruf ist auf **1 Stunde** beschränkt, um Batterie zu schonen und API-Limits einzuhalten.
4. **Angepasste Benutzeroberfläche (UI)**:
   ...
   - Die ursprünglichen Platzhalter-Buttons in den Ecken ("A", "B", "C", "D") und der "Sleep"-Button in der Mitte wurden für ein sauberes Erscheinungsbild entfernt, während die Touch-Logik im Hintergrund als Grundlage für zukünftige Interaktionen intakt blieb.
